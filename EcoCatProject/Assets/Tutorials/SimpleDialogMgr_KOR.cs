...
public class SimpleDialogMgr_KOR : MonoBehaviour
{
    ...
    public void OpenDialogue1()
    {
        ...
        if (clickCount > 9)
        {
            ...
            GameObject.Find("Canvas").transform.Find("Portrait").gameObject.SetActive(true);
            GameObject.Find("Canvas").transform.Find("ClickHere").gameObject.SetActive(false);
        }
    }

    public void OpenDialogue_PolarBearIntro1()
    {
        clickCount++;
        if (clickCount >= 1)
        {
    ...
    //void Update()
    //{//"난 키보드로 누르고싶어 마우스로 누르기 귀찮단말이야"라는 사람은 여기참고
    //    if (nextButton.activeSelf == true)
    //    {
    //        if (Input.GetKeyDown(KeyCode.Space))//GetKeyDown 으로 한번만 입력되게 하는게 중요
    //        {
    //            skipButtonControl();
    //        }
    //    }
